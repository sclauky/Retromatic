...
public class SudokuDatabaseHelper {
    ...
    // Migration: ajoute la colonne difficulty si la base provient d'une ancienne version.
    private static void ensureDifficultyColumn(Connection conn) throws Exception {
        ...
        try (ResultSet rs = metaData.getColumns(null, null, "grids", "difficulty")) {
            if (!rs.next()) {
                conn.createStatement().execute("ALTER TABLE grids ADD COLUMN difficulty TEXT NOT NULL DEFAULT 'FACILE'");
            }
        }
    }

    private static void addDefaultGrids(Connection conn) throws Exception {
        // Format : {puzzle, solution, difficulty}
        String[][] grids = {
            // === FACILE (3 grilles) ===
            {
